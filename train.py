...
def train_one_epoch(
    ...
    for step, batch in enumerate(pbar):
        ...
        # Distillation soft targets (Direction #5; Lesson #60). Present only
        # when the dataset is configured with a soft_target_index. We DROP
        # soft targets when Mixup/CutMix is active in this step, because the
        # mixed image's teacher distribution is not well-defined and forcing
        # KL on a mixed sample creates inconsistent gradients (mixup labels
        # are linear combinations; soft-target KL is not).
        soft_targets = batch.get("soft_target")
        soft_target_mask = batch.get("soft_target_mask")
        if soft_targets is not None:
            soft_targets = soft_targets.to(device)
            soft_target_mask = soft_target_mask.to(device)
        ...
        with autocast():
            ...
            if lam < 1.0:
                ...
                # Distillation off in mixup steps (see comment above on batch).
