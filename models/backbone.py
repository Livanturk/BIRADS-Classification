...
class BackboneFeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        # Direction #1 (Lesson #62 follow-up): if a domain-pretrained encoder
        # checkpoint is provided, OVERWRITE the timm-loaded ImageNet weights
        # with our mammography-pretrained weights. This is how MAE features
        # from `tools/train_mae.py` are consumed at fine-tune time.
        if pretrained_checkpoint:
            import os as _os
            if not _os.path.isfile(pretrained_checkpoint):
                raise FileNotFoundError(
                    f"pretrained_checkpoint not found: {pretrained_checkpoint}"
                )
            print(f"[BACKBONE] Loading domain-pretrained checkpoint: {pretrained_checkpoint}")
            state = torch.load(pretrained_checkpoint, map_location="cpu", weights_only=False)
            # Tolerate either a raw state_dict or a dict with a "state_dict" key.
            if isinstance(state, dict) and "state_dict" in state:
                state = state["state_dict"]
            # If the checkpoint was saved from inside DataParallel, strip the prefix.
            if any(k.startswith("module.") for k in state.keys()):
                state = {k[len("module."):]: v for k, v in state.items() if k.startswith("module.")}
            missing, unexpected = self.backbone.load_state_dict(state, strict=False)
            n_missing = len(missing)
            n_unexpected = len(unexpected)
            n_loaded = sum(1 for k in state if k in dict(self.backbone.named_parameters()))
            print(f"[BACKBONE]   loaded={n_loaded} missing={n_missing} unexpected={n_unexpected}")
            if n_missing > 0:
                print(f"[BACKBONE]   missing (showing 5): {missing[:5]}")
            if n_unexpected > 0:
                print(f"[BACKBONE]   unexpected (showing 5): {unexpected[:5]}")
